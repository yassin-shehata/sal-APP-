...
# Add canvas to layout or set its geometry manually
        ...
        self.STDValues = [[float('nan'), float('nan'), float('nan'), float('nan')], [50, 100, 1000, 5000]]
    ...
    def main(self):
        ...
        self.STD50MeasurementButton.clicked.connect(self.STD50MeasurementButtonPushed)
        ...
        if os.path.isfile(self.rawFileName):
            ...
            if msg_box.clickedButton() == delete_btn:
                ...
                if not self.std_data.empty:
                    self.STD50ppmEditField.setPlainText(str(float(self.std_data.iloc[-1, 5])))
                    ...
                    self.STDValues[0] = [
                        float(self.STD50ppmEditField.toPlainText()),
    ...
    QMessageBox.Yes | QMessageBox.No)
        ...
        if timer_choice == QMessageBox.Yes:
            dlg = QProgressDialog("Preparing to setup timer", "Stop Timexr", 0, 1800, self)
        ...
        self.STD50MeasurementButton.setEnabled(True)
    ...
    def CalibrationCurveFittingButtonPushed(self):
        ...
        self.axes.set_xlabel("STD order  (1 = 50 ppm … 4 = 5000 ppm)")
    ...
    def SetEnableButtonsIon(self, mode):
        state = mode.lower() == "on"
        self.STD50MeasurementButton.setEnabled(state)
    ...
    def STD50MeasurementButtonPushed(self):
        ...
        std_check = QMessageBox.question(
            self, "STD Check", "Is 50 ppm standard solution correct??",
            QMessageBox.Yes | QMessageBox.No
        )
        if std_check == QMessageBox.No:
            self.txt_system_note.setText("Before collecting the 50 ppm standard solution, place the correct standard solution.")
        ...
        self.lbl_progress_status.setText("Calculating data for 50 ppm STD solution")
        time.sleep(0.5)

        std_val = np.mean(self.rawData)
        self.STD50ppmEditField.setVisible(True)
        self.STD50ppmEditField.setPlainText(f"{std_val:.2f}")

        self.STD50ppmEditField.setStyleSheet("color: black; background-color: white;")  # <-- Add here

        self.STD50ppm = std_val
       ...
       ## if any(not np.isnan(val) for val in self.STDValues[0]):
        ...
        self.bar_progress.setVisible(False)
        self.lbl_progress_status.setVisible(False)
        self.txt_system_note.setText("The value of 50 ppm standard solution has been successfully collected.")
    ...
    # Sample Data Row
        ...
        header_std = [
            ...
            "STD 50ppm", "STD 100ppm", "STD 1000ppm", "STD 5000ppm",
    ...
    def recalculateButtonPushed(self):
        ...
        header_std = [ "Date", "ProjectName", "SampleNo", "SampleID", "Replication", "STD 50ppm", "STD 100ppm", "STD 1000ppm", "STD 5000ppm","STD check (100ppm)", "STD check potential (mV)"]
